# NOTE (BMCBL vendored copy)
#
# This directory contains a non-official, modified copy of `libpnet` (`pnet` crate),
# vendored for `Better-Minecraft-Bedrock-Launcher` / `EasyTier-BMCBL`.
# It is NOT the upstream `libpnet` repository.
#
# 中文说明: README_CN.md
#
# Upstream: https://github.com/libpnet/libpnet
#
# Why this exists:
# - On Windows, upstream `pnet`'s `std` feature can pull in `pnet_datalink`, which links
#   against WinPcap/Npcap (`Packet.lib` / `Packet.dll`). This app intentionally avoids
#   that dependency.
#
# What is changed (high level):
# - `pnet_datalink` is no longer implied by the `std` feature; it is behind an explicit
#   feature flag: `datalink`.
# - Add empty `nightly` / `clippy` features to avoid `unexpected_cfgs` build errors in
#   downstream setups where those feature names are referenced via `cfg_attr`.
# - Minor `cfg` fixups related to feature gating.
#
# Licensing and attribution:
# - This copy retains upstream license files (`LICENSE-APACHE`, `LICENSE-MIT`) and the
#   original source code copyright headers.
# - `libpnet` is dual-licensed under MIT OR Apache-2.0; see the license files above.
#
